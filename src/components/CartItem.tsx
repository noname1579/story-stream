...
const CartItem: React.FC<CartItemProps> = ({ item }) => {
  ...
  return (
    <div className="flex flex-col sm:flex-row items-center py-4 border-b border-gray-200">
      <div className="h-40 w-40 flex-shrink-0 overflow-hidden rounded-md">
        <img
          src={book.coverImage}
          alt={book.title}
      ...
      <div className="flex items-center border border-gray-200 rounded mt-2 sm:mt-0">
        <button 
      ...
      <div className="ml-4 text-base font-medium text-gray-900 mt-2 sm:mt-0">
        {(book.price * quantity).toFixed(2)} ₽
      </div>

      <button
        onClick={() => removeFromCart(book.id)}
        className="ml-4 text-gray-400 hover:text-red-500 transition-colors mt-2 sm:mt-0"
        title="Удалить"
      >
